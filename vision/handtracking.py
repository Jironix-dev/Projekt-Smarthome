"""Hand-Tracking und UI-Rendering.

Dieses Modul enthält die `HandTracker`-Klasse, die MediaPipe-Handlandmarks
aus der Kamera liest, Cursor-Positionen (Daumen) bestimmt, Pinch-Gesten
erkennt und die UI steuert. Zusätzlich übernimmt sie das Zeichnen des
gemeinsamen Fensters für Tracking und UI.
"""
...
class HandTracker:
    ...
    def run(self):
        while True:
            ...
            if self.ui:
                if self.ui.current_view == "HOME":
                    ...
                    if not self.menu_button.is_open:
                        # detect hovered room based on hand cursor if available, otherwise no hover
                        hovered = None
                        if hasattr(self, "cursor_x") and self.cursor_x is not None:
                            for room, shape in self.ui.room_zones.items():
                                if self.ui.is_point_in_room(self.cursor_x, self.cursor_y, room):
                                    hovered = room
                                    break

                        for room, shape in self.ui.room_zones.items():
                            # highlight if selected or hovered; otherwise leave invisible
                            is_selected = (room == self.ui.selected_room) or (room == hovered)
                            if is_selected:
                                self.ui.draw_room(room, shape, self.ui.rooms[room], True)
                        # Draw room labels (use UI helper so appearance matches UI.run)
                        for room, shape in self.ui.room_zones.items():
                            is_selected = (room == self.ui.selected_room) or (room == hovered)
                            try:
            ...
            # Menü-Knopf wird in HOME gezeichnet; in anderen Views wird er von der View selbst gezeichnet.
            if self.ui and self.ui.current_view == "HOME":
                self.menu_button.draw(self.screen)

            # Wenn das Menü offen ist: draw logout/exit in allen Views (funktionalität konsistent halten)
            if self.menu_button.is_open:
                ...
                if pinch_active and not self.last_pinch_active:
                    ...
                    if self.menu_button.is_open:
                        ...
                        # Wenn außerhalb aller Menü-Buttons gepincht wird: Menü schließen
                            ...
                            self.menu_button.toggle()
                            self.logger.log(
                                user=f"User {self.user_id}", action="Menü wurde geschlossen"
                            )

                    # Menü ist NICHT offen: Normal funktionieren
                    else:
                        # Zurück-Button in allen Room-Views
                        if self.ui:
                            if self.ui.current_view == "SCHLAFZIMMER":
                                if self.ui.schlafzimmer_view.back_button.is_clicked(*cursor_pos):
                                    self.ui.current_view = "HOME"
                                    self.logger.log(
                                        user=f"User {self.user_id}", action="Zurück zur HOME-View"
                                    )
                            elif self.ui.current_view == "WOHNZIMMER":
                                if self.ui.wohnzimmer_view.back_button.is_clicked(*cursor_pos):
                                    self.ui.current_view = "HOME"
                                    self.logger.log(
                                        user=f"User {self.user_id}", action="Zurück zur HOME-View"
                                    )
                            elif self.ui.current_view == "BADEZIMMER":
                                if self.ui.badezimmer_view.back_button.is_clicked(*cursor_pos):
                                    self.ui.current_view = "HOME"
                                    self.logger.log(
                                        user=f"User {self.user_id}", action="Zurück zur HOME-View"
                                    )
                            elif self.ui.current_view == "KUECHE":
                                if self.ui.kueche_view.back_button.is_clicked(*cursor_pos):
                                    self.ui.current_view = "HOME"
                                    self.logger.log(
                                        user=f"User {self.user_id}", action="Zurück zur HOME-View"
                                    )
                        ...
                        if self.menu_button.is_clicked(*cursor_pos):
                            ...
                            menu_state = "geöffnet" if self.menu_button.is_open else "geschlossen"
                            self.logger.log(
                                user=f"User {self.user_id}", action=f"Menü wurde {menu_state}"
                            )
                            ...
                            for room, shape in self.ui.room_zones.items():
                                if self.ui.is_point_in_room(self.cursor_x, self.cursor_y, room):
                                    if room == "Schlafzimmer":
                                        ...
                                        self.logger.log(
                                            user=f"User {self.user_id}", action=f"Zu {room} gewechselt"
                                        )
                                    elif room == "Wohnzimmer":
                                        # Wohnzimmer: Wechsel zur Detail-View
                                        self.ui.current_view = "WOHNZIMMER"
                                        self.logger.log(
                                            user=f"User {self.user_id}", action=f"Zu {room} gewechselt"
                                        )
                                    elif room == "Badezimmer":
                                        # Badezimmer: Wechsel zur Detail-View
                                        self.ui.current_view = "BADEZIMMER"
                                        self.logger.log(
                                            user=f"User {self.user_id}", action=f"Zu {room} gewechselt"
                                        )
                                    elif room == "Kueche":
                                        # Küche: Wechsel zur Detail-View
                                        ...
                                        room_state = (
                                            "eingeschaltet" if self.ui.rooms[room] else "ausgeschaltet"
                                        )
                                        self.logger.log(
                                            user=f"User {self.user_id}", action=f"{room} wurde {room_state}"
                                        )
